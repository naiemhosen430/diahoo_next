...
export default function PostFooter({likes, comments, postid, postOwnerid, postOwnerName}) {
  ...
  useEffect(() => {
    ...
    for (const nestedArray of postFooter.likes) {
      alreadyLiked = nestedArray.find(like => like.likeduserid === data.userId);
      if (alreadyLiked) {
        break;
      }
    }
  ...
  const likepost = async () => {
    ...
    await api.post(`/api/v1/post/like/${data.userId}/${postid}/${postOwnerid}`)
      .then((response) => {
        if (response) {
    ...
    const dislikepost = async () => {
      ...
      await api.post(`/api/v1/post/dislike/${data.userId}/${postid}/${postOwnerid}`)
        .then((response) => {
          if (response) {
            if (likes.length === 0) {
      ...
      <div className='p-4 flex text-fuchsia-50 text-center'>
          <div className='w-1/3 text-center px-4'>
            <h1 className='text-md text-center text-white'>{likeConter}</h1>
              { like && <AiOutlineHeart className='block w-full text-4xl p-2 my-2 bg-slate-800 opacity-60 hover:opacity-100 rounded-xl cursor-pointer' onClick={likepost} />}
              { dislike && <AiFillHeart className='block w-full text-4xl p-2 my-2 bg-slate-800 opacity-60 hover:opacity-100 rounded-xl cursor-pointer' onClick={dislikepost} />}
          </div>
          <div className='w-1/3 text-center px-4'>
              <h1 className='text-md text-center text-white'>{postFooter.comments.length}</h1>
              <BiSolidComment 
