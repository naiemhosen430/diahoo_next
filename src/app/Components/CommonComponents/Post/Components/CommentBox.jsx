...
export default function CommentBox({ closeCommentBox, allComments, postid, postOwnerName }) {
  ...
  const handleSubmit = async () => {
    ...
    await api.post(`/api/v1/post/comment/submit/${data.userId}/${postid}`, comment)
      .then((response) => {
        if (response) {
        }
      })
  ...
  return (
    <div className='backdrop-blur-xl bg-b fixed top-0 z-20 w-2/5 h-screen' onClick={closeCommentBox}>
        ...
        <div className='p-4' onClick={safeBox}>
          <div className=' bg-black p-2 fixed top-0 w-full'>
            <div onClick={safeBox} className='flex'>
              ...
              <div className='w-5/5'>
                <h1 className='text-white'>{postOwnerName}'s post. Total comments ({commentData.length})</h1>
              </div>
            </div>
          </div>
          ...
          <div>
            <div>
              {commentData.length === 0 ? (
                <h1 className='text-slate-500 text-center font-bold p-2 py-20'>No comment yet!</h1>
              ) : (
                commentData.map(commentItem => (
                  <div key={commentItem.key} className='p-2 my-4 shadow-xl rounded-xl hover:bg-slate-700 flex items-center'>
                    <div className='w-1/6'>
                      <img src={commentItem.commenterInfo.data.profilePicture} alt="Profile" />
                    </div>
                    <div className='w-5/6'>
                      <div className='flex items-center'>
                        <h4 className='text-white'>{commentItem.commenterInfo.data.fullname}</h4>
                      </div>
                      <div className='flex items-center'>
                        <h6 className='text-slate-400'>{commentItem[0].commenttext}</h6>
                      </div>
                    </div>
                  </div>
                ))
